import React from 'react';
import clsx from 'clsx';
import Link from './shared/Link';
import styles from './submenu.module.css';

import { AiIcon, About, Contact, Feedback, Forum, Nextflow, Support } from './shared/icons';

const links = {
  Help: [
    ['Forum', 'https://community.seqera.io', Forum],
    ['Support Portal', 'https://support.seqera.io', Support],
    ['Nextflow Slack', 'https://www.nextflow.io/slack-invite.html', Nextflow],
    ['Seqera AI', '/ask-ai', AiIcon],
  ],
  Company: [
    ['Seqera.io', '/about/', About],
    ['Contact us', '/contact-us/', Contact],
    ['Submit Feedback', '/feedback/', Feedback],
  ],
};

function isActive(id) {
  if (typeof window === 'undefined') return false;
  return window.location.pathname === id;
}

const LinkItem = ({ to, children }) => {
  let href = to;
  let sameTab = false;
  if (!to.startsWith('http')) {
    href = `https://seqera.io${to}`;
  }
  if (href.includes('seqera.io')) sameTab = true;
  return (
    <Link
      sameTab={sameTab}
      to={href}
      className={clsx(
        styles.submenuLink,
        'flex items-center rounded-md px-2 text-[14px]',
        'transition-all duration-500 ease-in-out hover:no-underline',
        'h-[37px]',
        { 'bg-brand-200': isActive(href) }
      )}
    >
      {children}
    </Link>
  );
};

const Column = ({ title, links }) => {
  return (
    <div className="tw-wrapper remove-infirma ">
    <div className="flex min-w-[200px] flex-col items-start">
      <p className="font-display mt-[2px] mb-[2px] text-[.8rem] font-semibold px-2">{title}</p>
      <ul className="mx-2 mt-2 mb-0 list-none !p-0">
        {links.map(([label, href, Icon]) => (
          <li key={label}>
            <LinkItem to={href}>
              <div className={clsx(styles.iconItem)}>
                <Icon className="" />
              </div>
              {label}
            </LinkItem>
          </li>
        ))}
      </ul>
    </div>
    </div>
  );
};

const Resources = () => {
  return (
    <div className="flex p-2 pl-4">
      <div className="flex flex-auto">
        {Object.entries(links).map(([title, links]) => (
          <Column key={title} title={title} links={links} />
        ))}
      </div>
    </div>
  );
};

export default Resources;
